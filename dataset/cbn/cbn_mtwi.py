...
mtwi_root_dir = './data/MTWI2018/'
mtwi_train_data_dir = mtwi_root_dir + 'image_train/'
mtwi_train_gt_dir = mtwi_root_dir + 'txt_train/'

mtwi_test_data_dir = mtwi_root_dir + 'icpr_mtwi_task2/image_test/'
mtwi_test_gt_dir = mtwi_root_dir + ''
...
def get_ann(img, gt_path):
    ...
    for line in lines:
        ...
        gt = line.split(',')
        word = ''
        for kk in range(len(gt)-8):
            tmp = gt[kk+8].replace('\r', '').replace('\n', '' )
            if tmp == '@':
                tmp = ','
            word += tmp + ','
        word = word[:-1]
...
def scale_aligned_shortv2(img, short_size=768):
    h, w = img.shape[0:2]
    ratio = h*1.0/w
    scale = short_size * 1.0 / min(h, w)
    h = int(h * scale + 0.5)
...
def resize_range(image, size=(640, 1280)):
    h, w, _ = image.shape
...
class CBN_MTWI(data.Dataset):
    def __init__(self,
        ...
        if split == 'train':
            data_dirs = [mtwi_train_data_dir]
            gt_dirs = [mtwi_train_gt_dir]
        elif split == 'test':
            data_dirs = [mtwi_test_data_dir]
            gt_dirs = [mtwi_test_gt_dir]
        else:
    ...
    def prepare_test_data(self, index):
        ...
        img = resize_range(img)  # current used
        #img = scale_aligned_shortv2(img, 768)
