...
class ResultFormat(object):
    ...
    def write_result(self, img_name, outputs):
        ...
        elif 'IC19' in self.data_type:
            self._write_result_ic19(img_name, outputs)
        elif 'IC17' in self.data_type:
            self._write_result_ic17(img_name, outputs)
        elif 'MTWI' in self.data_type:
            self._write_result_mtwi(img_name, outputs)

    def _write_result_mtwi(self, image_name, outputs):
    ...
    def _write_result_ic17(self, img_name, outputs):
        ...
        for i, bbox in enumerate(bboxes):
            ...
            if words is None:
                ...
                #pdb.set_trace()
                score = 1.0
                #line = '%d,%d,%d,%d,%d,%d,%d,%d' % tuple(values) + ',%f\n' % outputs['scores'][i]
                line = '%d,%d,%d,%d,%d,%d,%d,%d' % tuple(values) + ',%f\n' % score 
                lines.append(line)
    ...
    def _write_result_ic19(self, img_name, outputs):
        assert self.result_path.endswith(
            '.zip'), 'Error: ic15 result should be a zip file!'
